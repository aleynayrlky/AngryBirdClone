...
//  AngryBirdClone
//
//  Created by Aleyna Yerlikaya on 26.03.2024.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    enum ColliderType: UInt32 {
        case Bird = 1
        case Box = 2
        //1, 2, 4, 8, 16, 32 şeklinde artmalı
    }
    ...
    override func didMove(to view: SKView) {
        ...
        bird2.position = CGPoint(x: -self.frame.width / 2.8, y: -self.frame.height / 6)
        ...
        scoreLabel.zPosition = 2 //kuşun üstüne denk gelsin diye
        self.addChild(scoreLabel)
        
        
    }
    
    func didBegin(_ contact: SKPhysicsContact) { //oluşturduğumuz kategorilerden birisi birbirine değerse çağrılacak
    ...
    //bu üçünde de direkt dokunduğu noktayı vererek işlem yapıyor
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) { //kullanıcı elini oynattı
        if gameStarted == false {
            if let touch = touches.first{
                let touchLocation = touch.location(in: self)
                let touchNodes = nodes(at: touchLocation)
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) { //ekrana dokunmayı bitirdi
        if gameStarted == false {
            if let touch = touches.first{
                let touchLocation = touch.location(in: self)
                let touchNodes = nodes(at: touchLocation)
    ...
    override func touchesCancelled(_ touches: Set<UITouch>, with event: UIEvent?) { //dokunamktan vazgeçti
